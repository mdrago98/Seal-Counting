...
class SealDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, image_path:str, locations: DataFrame = None):
        if locations is None:
            locations = self.get_seal_locations('pixel_coord.xlsx', sheet_name='PixelCoordinates', bbox_size=80)
    ...
    def get_seal_locations(self, seal_loc_file: str, sheet_name='PixelCoordinates', bbox_size=80) -> DataFrame:
        radius = SEAL_BOX_SIZE/2
        locations = read_excel(seal_loc_file, sheet_name=sheet_name)[['tiff_file', 'layer_name', 'x_pixel', 'y_pixel']]
        locations['bbox_1'] = locations.apply(lambda x: [x.x_pixel - radius, x.y_pixel - radius] if notnull(x.pixel) else [])
        locations['bbox_2'] = locations.apply(lambda x: [x.x_pixel + radius, x.y_pixel + radius] if notnull(x.pixel) else [])
        return locations
