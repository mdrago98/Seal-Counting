...
def draw_plot_func(
    ...
    if true_p_bar != "":
        ...
        plt.barh(
            range(n_classes), fp_sorted, align="center", color="crimson", label="False Positive"
        )
        plt.barh(
            range(n_classes),
            tp_sorted,
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                ...
                text = (
                    "Class [" + str(class_index) + "/" + str(n_classes) + "]: " + class_name + " "
                )
                ...
                cv2.putText(
                    img_cumulative, class_name, (bb[0], bb[1] - 5), font, 0.6, color, 1, cv2.LINE_AA
                )
                # show image
...
if draw_plot:
    ...
    plot_title += (
        "(" + str(len(ground_truth_files_list)) + " files and " + str(n_classes) + " classes)"
    )
