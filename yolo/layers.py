...
def DarknetConv2D(*args, **kwargs):
    """Wrapper to set darknet_backbone parameters for Convolution2D."""
...
def DarknetConv2D_BN_Leaky(*args, **kwargs):
    """darknet_backbone Convolution2D followed by BatchNormalization and LeakyReLU."""
    no_bias_kwargs = {"use_bias": False}
    no_bias_kwargs.update(kwargs)
    return compose(
...
def Spp_Conv2D_BN_Leaky(x, num_filters):
    ...
    y = compose(Concatenate(), DarknetConv2D_BN_Leaky(num_filters, (1, 1)))([y1, y2, y3, x])
    return y
...
def make_spp_last_layers(x, num_filters, out_filters, predict_filters=None, predict_id="1"):
    ...
    x = compose(
        DarknetConv2D_BN_Leaky(num_filters * 2, (3, 3)), DarknetConv2D_BN_Leaky(num_filters, (1, 1))
    )(x)
...
def make_depthwise_separable_last_layers(
    ...
    x = compose(
        ...
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=num_filters * 2, kernel_size=(3, 3), block_id_str=block_id_str + "_1"
        ),
        DarknetConv2D_BN_Leaky(num_filters, (1, 1)),
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=num_filters * 2, kernel_size=(3, 3), block_id_str=block_id_str + "_2"
        ),
        DarknetConv2D_BN_Leaky(num_filters, (1, 1)),
    )(x)
    ...
    y = compose(
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=predict_filters, kernel_size=(3, 3), block_id_str=block_id_str + "_3"
        ),
        DarknetConv2D(out_filters, (1, 1), name="predict_conv_" + predict_id),
    )(x)
    return x, y
...
def tiny_yolo3_predictions(feature_maps, feature_channel_nums, num_anchors, num_classes):
    ...
    # upsample fpn merge for feature map 1 & 2
    x2 = compose(DarknetConv2D_BN_Leaky(f2_channel_num // 2, (1, 1)), UpSampling2D(2))(x1)
