...
def get_logger(output_dir):
    ...
    file_title = os.path.join(output_dir, "logs", "session.log")
    Path(file_title).parent.mkdir(exist_ok=True, parents=True)
...
def calculate_display_data(prediction_file, classes_file, img_width, img_height, out):
    ...
    indices = {
        item: ind
        for ind, item in enumerate([item.strip() for item in open(classes_file).readlines()])
    }
    ...
    fr = pd.DataFrame(
        rows, columns=["Image", "Object Name", "Object Index", "bx", "by", "bw", "bh"],
    )
