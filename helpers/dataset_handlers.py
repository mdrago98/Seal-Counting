...
def read_example(
    ...
    y_train = tf.stack(
        [
            ...
            for feature in ['image/object/bbox/xmin',
...
def save_tfr(data, output_folder, dataset_name, test_size=None, trainer=None):
    ...
    data.to_csv(
        os.path.join('../yolo', 'Data', 'TFRecords', 'full_data.csv'), index=False
    )
    ...
    if test_size:
        ...
        training_frame.to_csv(
            os.path.join('../yolo', 'Data', 'TFRecords', 'training_data.csv'),
            index=False,
        )
        test_frame.to_csv(
            os.path.join('../yolo', 'Data', 'TFRecords', 'test_data.csv'),
            index=False,
        )
        training_path = str(
