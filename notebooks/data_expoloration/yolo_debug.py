...
def DarknetConv2D(*args, **kwargs):
    """Wrapper to set darknet_backbone weight regularizer for Convolution2D."""
...
def resblock_body(x, num_filters, num_blocks):
    ...
    # darknet_backbone uses left and top padding instead of 'same' mode
...
def train(
    ...
    log_dir: str = "logs",
    val_split: float = 0.2,
    optimiser=Adam(lr=learning_rate, decay=5e-4),
):
    logging = TensorBoard(
        log_dir=log_dir,
        histogram_freq=0,
    ...
    checkpoint = ModelCheckpoint(
        path.join(log_dir, "ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5"),
        monitor="val_loss",
        verbose=1,
    ...
    reduce_lr = ReduceLROnPlateau(
        monitor="val_loss", factor=0.5, patience=10, verbose=1, cooldown=0, min_lr=1e-10
    )
